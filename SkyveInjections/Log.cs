    ...
    public static class Log {
        ...
        public static Stopwatch GetSharedTimer() {
            var asm = AppDomain.CurrentDomain.GetAssemblies()
                .FirstOrDefault(_asm => _asm.GetName().Name == "SkyveIPatch");
            var t = asm?.GetType("SkyveIPatch.Patches.UnityLoggerPatch", throwOnError: false);
        ...
        static Log() {
            try {
                ...
                TryDeleteFile(Path.Combine(dir, "SkyveMod.log"));
                TryDeleteFile(Path.Combine(dir, "SkyveInjections.log"));
